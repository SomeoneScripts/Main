...
function Library:GetIcon(Name)
	...
	if type(Icon) == "string" and Icon:lower():find("https://") then
        local FileName = Name:gsub("[^%w]", ""):sub(-30) .. ".png"
        local Path = FileName
        if isfile(Path) then
            local Icon = getcustomasset(Path)
            delfile(Path)
            return Icon
        end
        local Success, Bytes = pcall(game.HttpGet, game, Icon)
        if Success then
