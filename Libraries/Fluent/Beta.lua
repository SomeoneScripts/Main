...
loadstring(game:HttpGet("https://raw.githubusercontent.com/SomeoneScripts/Main/refs/heads/main/TranslationSystem/Translate.lua"))()
function Translate(text)
    local textLower = text:lower()
    local final = text
    if Translations and S and S["Select The Language"] and S["Translate"] then
        local lang = Translations[S["Select The Language"]]
    ...
    return final:gsub("(%S)(%S*)", function(a,b) return a:upper()..b end)
end
...
table.sort(Themes.Names, function(a, b) return a:lower() < b:lower() end)
...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:AddSection(SectionTitle, SectionIcon)
			...
			local Section = { Type = "Section" }
			SectionTitle = typeof(SectionTitle) == "string" and SectionTitle or SectionTitle.Title
			local Icon = SectionIcon
			if not fischbypass then 
				if Library:GetIcon(Icon) then
					Icon = Library:GetIcon(Icon)
...
Components.Dialog = (function()
	...
	function Dialog:Create()
		...
		function NewDialog:Button(Title, Callback)
			...
			Button.Title.Text = Translate(Title)
